...
export default function Stage3Tag_ENG_TEST({ StudentID, headers, colorTag }) {
  ...
  useEffect(() => {
    async function fetchData() {
      const data = await StudentEnglishScore(StudentID);
      setEnglishData(data);
    }
  ...
  // ป้องกัน null
  const tokenENG = englishData?.tokenENG ?? "กำลังสร้างรหัสผ่าน";
  const status = englishData?.status ?? "กำลังดำเนินการ";
  ...
  return (
    <div>
      <h3 className="text-lg sm:text-xl font-semibold text-[#1a365d] mb-3 sm:mb-4 font-sukhumvit px-2 sm:px-0">
        ข้อมูลสำหรับการทดสอบทักษะภาษาอังกฤษ
      </h3>
